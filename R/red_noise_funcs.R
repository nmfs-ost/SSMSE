...
calc_comp_var <- function(data_obs, data_exp, bins, fleets = NULL, years = NULL, seasons = NULL, merge_genders = TRUE, genders = NULL, merge_seasons = TRUE, merge_fleets = FALSE) {
  ...
  data_obs <- data_obs[is.element(data_obs[["year"]], years) & is.element(data_obs[["fleet"]], fleets) & is.element(data_obs[["Seas"]], seasons) & is.element(data_obs[["sex"]], genders) & data_obs[["year"]] > 0, ]
  data_exp <- data_exp[is.element(data_exp[["year"]], years) & is.element(data_exp[["fleet"]], fleets) & is.element(data_exp[["Seas"]], seasons) & is.element(data_exp[["sex"]], genders) & data_exp[["year"]] > 0, ]
  ...
  for (i in 1:length(fleets)) {
    ...
    for (j in 1:length(genders)) {
      ...
      for (l in 1:length(seasons)) {
        ...
        sub_dat_obs <- data_obs[data_obs[["fleet"]] == fleets[i] & data_obs[["sex"]] == genders[j] & data_obs[["Seas"]] == seasons[l], ((length(data_obs[1, ]) - length(bins) + 1):length(data_obs[1, ]))]
        sub_dat_exp <- data_exp[data_exp[["fleet"]] == fleets[i] & data_exp[["sex"]] == genders[j] & data_obs[["Seas"]] == seasons[l], ((length(data_exp[1, ]) - length(bins) + 1):length(data_exp[1, ]))]
